...
def _main(argv, capsys, output):

    main(argv)

    outerr = capsys.readouterr()
    assert outerr.out.strip() == output

def test_onpass(capsys):
    argv = [
        "clean",
        "[[1,2,3], [1,4,9]]", "[4,5,6]",
        "-t", "D[1].values[0] == 4",
    ...
    _main(argv, capsys, 'PASSED')

def test_onfail(capsys):
    argv = [
        "verify",
        "[[1,2,3], [1,4,9]]", "[4,5,6]",
        "-t", "D[1].values[1] == 4",
    ...
    _main(argv, capsys, 'FAILED')

def test_onerror(capsys):
    argv = [
        "verify",
        "[[1,2,3], [1,4,9]]", "[4,5,6]",
        "-t", "D[1].values[1] = 4",
    ...
    _main(argv, capsys, 'ERROR')

def test_onpass_function(capsys):
    argv = [
        "verify",
        "[40, numpy.nan, 6.0]",
